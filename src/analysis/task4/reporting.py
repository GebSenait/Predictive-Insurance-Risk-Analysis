    ...
    def generate_markdown_report(
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            for task_name, task_results in all_results.items():
                ...
                if "model_results" in task_results:
                    comparison_df = self.create_model_comparison_table(
                        ...
                        task_type="regression" if "severity" in task_name or "premium" in task_name else "classification",
                    )
                    ...
                    if "best_model" in task_results:
                        ...
                        if "test_r2" in task_results["best_model"]:
                            f.write(f"- **Test R²**: {task_results['best_model']['test_r2']:.4f}\n")
                            f.write(f"- **Test RMSE**: {task_results['best_model']['test_rmse']:.2f}\n")
                        elif "test_f1" in task_results["best_model"]:
                            f.write(f"- **Test F1**: {task_results['best_model']['test_f1']:.4f}\n")
                            f.write(f"- **Test Accuracy**: {task_results['best_model']['test_accuracy']:.4f}\n")
                        f.write("\n")
            ...
            if "interpretability" in all_results:
                ...
                for model_name, interp_results in all_results["interpretability"].items():
                    f.write(f"#### {model_name}\n\n")
                    
                    if "top_features" in interp_results and interp_results["top_features"]:
                        f.write("| Rank | Feature | Combined Importance |\n")
                        f.write("|------|---------|---------------------|\n")
                        
                        for idx, feat in enumerate(interp_results["top_features"][:10], 1):
                            f.write(
                                f"| {idx} | {feat['feature']} | "
    ...
    def generate_business_interpretation(
        ...
        for idx, feat in enumerate(top_features[:5], 1):
            ...
            if "age" in feat_name.lower():
                interpretation += (
                    "   - Age is a significant risk factor. Consider age-based pricing tiers.\n"
                )
            elif "province" in feat_name.lower() or "postal" in feat_name.lower():
                interpretation += (
                    "   - Geographic location impacts risk. Consider regional pricing adjustments.\n"
                )
            elif "gender" in feat_name.lower():
                interpretation += (
                    "   - Note: Gender-based pricing may be restricted by regulations.\n"
                )
            elif "premium" in feat_name.lower():
                interpretation += (
                    "   - Premium level correlates with risk. Higher premiums may indicate higher risk segments.\n"
                )
            else:
                interpretation += (
                    f"   - This feature significantly influences {model_type} predictions.\n"
                )
            interpretation += "\n"
